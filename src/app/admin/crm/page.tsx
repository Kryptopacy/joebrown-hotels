    ...
    <div className="animate-fade-in-up space-y-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          ...
          { icon: RefreshCw, label: 'Returning Guests',    value: repeatGuests.length.toString(),       color: 'bg-brown-500/200' },
          { icon: TrendingUp,label: 'Lifetime Revenue',    value: `₦${totalRevenue.toLocaleString()}`,  color: 'bg-brown-500/200' },
          { icon: Star,      label: 'Avg Spend / Guest',   value: `₦${guestProfiles.length > 0 ? Math.round(totalRevenue / guestProfiles.length).toLocaleString() : 0}`, color: 'bg-brown-500/200' },
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-1.5 flex-wrap">
                      ...
                      {guest.types.has('Hotel') && guest.types.has('Restaurant & Lounge') ? (
                        <span className="shrink-0 text-[9px] bg-brown-500/20 text-purple-600 border border-brown-500/30 font-bold px-1.5 py-0.5 rounded-full uppercase tracking-wide">Both</span>
                      ) : guest.types.has('Hotel') ? (
                        <span className="shrink-0 text-[9px] bg-brown-500/20 text-blue-600 border border-brown-500/30 font-bold px-1.5 py-0.5 rounded-full uppercase tracking-wide">Hotel</span>
                      ) : (
                        <span className="shrink-0 text-[9px] bg-brown-500/20 text-orange-600 border border-brown-500/30 font-bold px-1.5 py-0.5 rounded-full uppercase tracking-wide">Restaurant & Lounge</span>
                      )}
                      {isRepeat && (
                        <span className="shrink-0 text-[9px] bg-brown-500/20 text-emerald-600 border border-brown-500/30 font-bold px-1.5 py-0.5 rounded-full uppercase tracking-wide">Returning</span>
                      )}
                    </div>
                  </div>
              ...
              <div className="bg-[#0D0501] border border-white/10 rounded-3xl shadow-sm p-6">
                <div className="flex items-start justify-between mb-4">
                  <div>
                    <div className="flex items-center gap-2 mb-1">
                      ...
                      {selected.visits.length > 1 && (
                        <span className="text-[10px] bg-brown-500/20 text-blue-600 border border-brown-500/30 font-bold px-2 py-0.5 rounded-full uppercase">Returning Guest</span>
                      )}
                    </div>
                  ...
                  <a
                    ...
                    className="flex items-center gap-2 bg-brown-500/200 hover:bg-emerald-600 text-white text-xs font-bold px-4 py-2.5 rounded-xl transition-colors shadow-sm"
                  >
                  ...
                  <div className="divide-y divide-white/5">
                    {selected.visits.map((v: any) => {
                      const statusMeta: Record<string, string> = {
                        ...
                        confirmed: 'bg-brown-500/20 text-brown-300 border-brown-500/30',
                        checked_in: 'bg-brown-500/20 text-brown-300 border-brown-500/30',
                        checked_out: 'bg-brown-500/20 text-brown-300 border-white/10',
                      };
                      const color = statusMeta[v.status] || 'bg-brown-500/20 text-brown-300 border-white/10';
            ...
            {repeatGuests.slice(0, 10).map((guest, idx) => (
              <div key={guest.phone} className="p-5 flex items-center justify-between hover:bg-[#1A0A02]/30 transition-colors">
                ...
                <div className="text-right">
                  ...
                  <span className="text-xs font-bold text-blue-600 bg-brown-500/20 border border-brown-500/30 rounded-full px-2 py-0.5 inline-block mt-0.5">↩ Returning</span>
                </div>
              </div>
            ))}
          </div>
