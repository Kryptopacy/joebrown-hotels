...
export default function AdminBookingsPage() {
  ...
  const getStatusColor = (status: string) => {
    switch(status) {
      case 'confirmed': return 'bg-brown-500/200/20 text-blue-300 border-blue-500/30';
      case 'checked_in': return 'bg-brown-500/200/20 text-purple-300 border-purple-500/30';
      case 'completed': return 'bg-brown-500/200/20 text-emerald-300 border-emerald-500/30';
      case 'cancelled': return 'bg-brown-500/200/20 text-red-300 border-red-500/30';
  ...
  const getPaymentStatusBadge = (status: string) => {
    switch(status) {
      case 'paid': return <span className="px-2 py-1 text-xs font-semibold rounded-full bg-brown-500/200/20 text-brown-300 border border-brown-500/30">Paid</span>;
      case 'unpaid': return <span className="px-2 py-1 text-xs font-semibold rounded-full bg-brown-500/200/20 text-brown-300 border border-brown-500/30">Unpaid</span>;
      default: return <span className="px-2 py-1 text-xs font-semibold rounded-full bg-white/10 text-brown-300 border border-white/10">Pending</span>;
    }
  };
              ...
              <div key={idx} className={`bg-[#0D0501] min-h-[120px] p-2 flex flex-col gap-1 transition-colors hover:bg-white/5 ${isToday ? 'ring-2 ring-inset ring-brown-500' : ''}`}>
                ...
                <div className="flex flex-col gap-1">
                  {dayBookings.slice(0, 3).map((b, i) => {
                    ...
                    if (b.status === 'confirmed') dotColor = 'bg-brown-500/200';
                    if (b.status === 'checked_in') dotColor = 'bg-brown-500/200';
                    if (b.status === 'pending') dotColor = 'bg-[#D4A373]';
                    if (b.status === 'completed') dotColor = 'bg-brown-500/200';
                    if (b.status === 'cancelled') dotColor = 'bg-brown-500/200 opacity-50';
              ...
              {rooms.map(room => (
                <tr key={room.id} className="group">
                  ...
                  {days.map((d, i) => {
                    ...
                    if (booking.status === 'confirmed') { bgCol = 'bg-brown-500/200/20'; textCol = 'text-blue-300'; borderCol = 'border-blue-500/30'; }
                    if (booking.status === 'checked_in') { bgCol = 'bg-brown-500/200/20'; textCol = 'text-purple-300'; borderCol = 'border-purple-500/30'; }
                    if (booking.status === 'pending') { bgCol = 'bg-[#D4A373]/20'; textCol = 'text-[#D4A373]'; borderCol = 'border-[#D4A373]/30'; }
                    if (booking.status === 'completed') { bgCol = 'bg-brown-500/200/20'; textCol = 'text-brown-300'; borderCol = 'border-emerald-500/30'; }
                    if (booking.status === 'cancelled') { bgCol = 'bg-brown-500/200/20 opacity-50'; textCol = 'text-brown-300'; borderCol = 'border-red-500/30'; }
