          ...
          <div className="divide-y divide-white/5">
            {requests.map((req) => (
              <div key={req.id} className={`p-6 flex flex-col sm:flex-row sm:items-center justify-between gap-4 transition-colors ${req.status === 'fulfilled' ? 'bg-white/5 opacity-70' : 'hover:bg-white/5'}`}>
                <div className="flex items-start gap-4">
                  <div className={`w-12 h-12 rounded-full flex items-center justify-center font-bold text-lg ${req.status === 'fulfilled' ? 'bg-brown-500/20 text-white/40' : 'bg-[#1A0A02] border border-[#D4A373]/30 text-[#D4A373]'}`}>
                    {req.room_number}
                  </div>
                  <div>
                    ...
                    <div className="flex items-center gap-2 text-xs font-medium text-white/50 mt-1">
                      ...
                      <span className={`px-2 py-0.5 rounded-full ${req.status === 'fulfilled' ? 'bg-brown-500/200/20 text-emerald-300' : 'bg-[#1A0A02] border border-[#D4A373]/30 text-[#D4A373]'} capitalize`}>
                        {req.status}
                      </span>
                    </div>
                  </div>
